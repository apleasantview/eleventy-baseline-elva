...
import {
	...
	error,
	getLocaleData,
	getTemplatePartChoices,
	COLLECTIONS_PATH,
	COLLECTIONS_TEMPLATE_PATH
} from './utils.js';
...
const listCollections = () => {
	...
	const header =
		'Label'.padEnd(24) + 'Layout'.padEnd(12) + 'Protected'.padEnd(12) + 'Search'.padEnd(12) + 'Feed'.padEnd(7);
...
const removeCollection = async () => {
	...
	if (
		!(await confirm({
			message: `Are you sure you want to remove '${config.label} (${choice})'? This will also delete all content in this collection across all locales.`
		}))
	) {
		return;
	}
