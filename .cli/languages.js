...
const addLanguage = async () => {
	...
	try {
		const defaultTranslations = JSON.parse(
			readFileSync(path.join(process.cwd(), 'src', '_data', 'translations', `${defaultLocale}.json`), 'utf-8')
		);
	...
	if (
		await confirm({ message: `Would you like to copy content from '${defaultLocale}' to '${newLanguage.locale}'?` })
	) {
...
const removeLanguage = async () => {
	...
	if (
		!(await confirm({ message: `Are you sure you want to remove '${result}'? This will also delete your content.` }))
	) {
		return;
	}
...
const listLanguages = () => {
	...
	languages.forEach((lang, index) => {
		const row =
			...
			lang.name.padEnd(15) +
			lang.key.padEnd(10) +
			lang.locale.padEnd(12) +
			lang.dir.padEnd(8) +
			(lang.default ? colors.green('Yes') : 'No');
		console.log(row);
	});
	console.log('-'.repeat(58));
};
